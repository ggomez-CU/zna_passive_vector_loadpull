...
#region Options
    ...
    config = MMICDriveUpConfig(r"C:\Users\grgo8200\Documents\GitHub\summer2025_loadpull_repo\data\PA_Spring2023\MMIC_driveup_config.json")
    now = datetime.now().strftime("%Y-%m-%d_%H_%M")
    output_dir = os.getcwd() + "\\data\\PA_Spring2023\\DriveUp" \
            + now + "_Freq" \
...
#region Set Instrumentation and Data to Frequency
        ...
        for power in tqdm(config.input_power_dBm, ascii=True, desc=string):
            ...
            for i in range(1):
                ...
                datatemp = {'PNA Power: '+ str(power) + '_' + str(i): 
                            {'load_gamma': 
                                {'real': loadpoint.real,
                                'imag': loadpoint.imag},
                            'wave data': rf_data,
        ...
        line[4*idx+3][0].set_data(np.full(len(plots['power_gain']),float(freq)),plots['power_gain'])
        line[4*idx+4][0].set_data(np.full(len(plots['power_gain']),float(freq)) ,plots['power_PAE'])
pna.set_power(-27)
pna.close()
loadtuner.close()
pm.close()