...
#region Options
    ...
    config = MMICCoupledLineFreqPowerConfig(r"C:\Users\grgo8200\Documents\GitHub\summer2025_loadpull_repo\data\coupledline_samplers\MMIC_coupledline_phase_config.json")
        ...
        for idx, bias in enumerate(config.dc_supply_config.sampler_bias):
            ...
            for loadpoint in tqdm(config.loadpoints, ascii=True, desc=string):
                ...
                for i in range(5):
                    ...
                    if i == 0:
                        time.sleep(1)
                        datatemp = {'Load Point: '+ str(loadpoint) + '_' + str(i) + '_' + str(bias): 
                                    {'load_gamma': 
                                        {'real': loadpoint.real,
                                        'imag': loadpoint.imag},
