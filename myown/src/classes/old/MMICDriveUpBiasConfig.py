...
class MMICDriveUpBiasConfig(TestConfig):
	...
	def __init__(self, config_file:str, Z0 = 50):
		...
		self.gatebiasvoltage = [float(num) for num in self.config_file_json["Gate Bias Voltages"]]
		...
		self.sampler_config = []
		self.sampler_config.append(SamplerConfig(self.config_file_json["Samplers"]["1"]["DMM Address"],
										   1,
                                           self.config_file_json["Samplers"]["1"]["Filter Number"]))
		self.sampler_config.append(SamplerConfig(self.config_file_json["Samplers"]["2"]["DMM Address"],
										   2,
